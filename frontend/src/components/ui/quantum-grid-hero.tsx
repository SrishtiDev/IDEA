...
export const CelestialOrbHero = ({ children }: { children?: React.ReactNode }) => {
  ...
  return (
    <div className="relative min-h-screen w-full flex items-center justify-center text-center bg-transparent overflow-hidden">
      <canvas
        ref={canvasRef}
        className="absolute top-0 left-0 w-full h-full z-0 opacity-80"
      />

      <main className="relative z-10 w-full px-4 mt-24">
        {children}
      </main>
    </div>
  );
